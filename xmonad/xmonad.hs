...
-- preset keybindings.
myLauncher = "~/.config/polybar/scripts/menu"
...
--
myWorkspaces = ["term","web","code","media"] ++ map show [5..9]
...
myManageHook = composeAll
    ...
    , className =? "discord"                  --> viewShift "media"
    
    , className =? ""                  --> viewShift "media"
...
resizable = avoidStruts
               $ renamed [Replace "Resizable"]
               $ addTopBar
			   $ windowNavigation
               $ myGaps
			   $ mouseResizableTile{draggerType = FixedDragger 10 10}
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
  ...
  , ((0, xK_Menu), sequence_ [broadcastMessage ToggleStruts, refresh, spawn "polybar-msg cmd toggle"])
  ...
  -- Quit xmonad.
  , ((modMask .|. shiftMask, xK_KP_Delete),
